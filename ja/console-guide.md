...
4. **[接続タイプ]**を選択します。サービスゲートウェイに割り当てられた IP でアクセスした際に、選択した対象と接続されます。
    ...
    * **[ユーザー定義エンドポイント]**: 他のユーザーが公開したリソース（ロードバランサー）に接続します。エンドポイントの公開者から受け取った**[対象サービス名]**を入力します。サービス名は `{region}.sep-{12桁の16進数}` の形式です（例: `kr1.sep-0a1b2c3d4e5f`）。
    ...
    !!! tip "ヒント"
        入力したサービス名は作成時に有効性が検証され、パブリッシャーの許可プロジェクトに含まれている場合にのみ接続できます。
...
8. **NAT IPの固定**を行うかどうかを選択します。
    * 通常は選択する必要はなく、選択した**サービス**でアクセス制御の設定が必要な場合のみ選択します。
    * 作成時にのみ選択可能で、変更はサポートされていません。
...
カスタムエンドポイントは、ユーザーが自身のリソース（ロードバランサー）をエンドポイントとして公開し、他のプロジェクトからサービスゲートウェイで接続できるよう共有する機能です。公開者は共有用の ***サービス名**（`service_name`）を発行して、接続を許可する対象に渡し、許可プロジェクトを直接管理します。

<a id="create-a-custom-endpoint"></a>
### ユーザー定義エンドポイントの作成 { #create-a-custom-endpoint }

ユーザー定義エンドポイントを作成する方法は次のとおりです。

1. **Network > Service Gateway** に移動し、**[ユーザー定義エンドポイント]** タブを選択します。
2. **[ユーザー定義エンドポイント作成]** ボタンをクリックすると、作成画面が表示されます。
3. エンドポイントに使用する **[名前]** を入力します。(255 文字以内、英数字・`-`・`_` のみ入力可能)
4. **[表示名]** を入力します。このエンドポイントに接続するサービスゲートウェイに表示される名前です。省略した場合は名前と同じ値が適用されます。
5. **[リソースタイプ]** と **[対象リソース]** を選択します。現在リソースタイプは **Load Balancer** のみサポートしており、対象リソースでエンドポイントとして公開するロードバランサーを選択します。
6. **[最大作成数]** を選択します。このエンドポイントで作成できるサービスゲートウェイの最大数です。
    * 制限なし: 数量の制限なく作成できます。1,000 を超える数量が必要な場合も、制限なしを選択します。
...
!!! tip "ヒント"
    ユーザー定義エンドポイントは、プロジェクトあたりデフォルトで最大 5 個まで作成できます。
...
### ユーザー定義エンドポイントの変更 { #modify-a-custom-endpoint }

**名前**、**表示名**、**最大作成数**、**説明**のみ変更できます。リソースタイプとターゲットリソースは変更することはできません。

1. **[ユーザー定義エンドポイント]** タブで変更するエンドポイントを選択します。
2. **[変更]** ボタンをクリックし、変更画面で変更する項目を変更します。

!!! tip "ヒント"
    最大作成数を減らしても、すでに作成されたサービスゲートウェイは維持されます。ただし、現在の数が最大作成数を超えている間は、新しいサービスゲートウェイを追加で作成することはできません。
...
!!! danger "注意"
    このエンドポイントを使用しているサービスゲートウェイが1つでもある場合は、削除することはできません。エンドポイントを削除すると、登録された許可プロジェクトも一緒に削除されます。

<a id="reissue-a-service-name"></a>
### サービス名の再発行 { #reissue-a-service-name }

共有したサービス名が外部に漏洩するなど、変更が必要な場合はサービス名を再発行できます。

1. **[ユーザー定義エンドポイント]** タブでエンドポイントを選択し、**[基本情報]** からサービス名の **[再発行]** ボタンをクリックします。
2. 確認ダイアログで **[再発行]** ボタンをクリックします。

!!! danger "注意"
    再発行すると、既存のサービス名は即座に廃棄され、以降は参照できなくなります。既存のサービス名で作成したサービスゲートウェイは引き続き正常に動作しますが、新たにサービスゲートウェイを作成する際は、再発行されたサービス名を使用する必要があります。

!!! tip "ヒント"
    サービス名の再発行は、エンドポイントを作成したプロジェクトのメンバー（オーナー）のみ実行できます。
...
2. **[追加]** ボタンをクリックし、**[許可範囲]** を選択します。
    ...
    * **[特定のプロジェクト]**: 許可するプロジェクトの **[テナントID]**（32桁の16進数）を入力します。
3. 必要に応じて **[説明]** を入力し、**[確認]** ボタンをクリックします。

!!! tip "ヒント"
    全許可（*）と特定のプロジェクトを同時に登録した場合、より範囲の狭い設定（特定のプロジェクト）が適用されます。これを利用すると、サービスを中断することなく許可範囲を切り替えることができます。たとえば、全許可（*）の状態で特定のプロジェクトを追加した後、全許可（*）を削除すると、接続を中断することなく特定のプロジェクトのみを許可する設定に切り替わります。

既存の許可対象は **[説明]** のみ変更できます。許可範囲とテナントIDは変更することはできません。許可対象を削除するには、リストから対象を選択し、**[削除]** ボタンをクリックします。

<a id="check-usage-status"></a>
### 使用状況の確認 { #check-usage-status }

エンドポイントの詳細情報の **[使用状況]** タブで、このエンドポイントに接続しているサービスゲートウェイの一覧を確認できます。(読み取り専用)
...
!!! tip "ヒント"
    オブジェクトストレージのAPI URLアドレスは、コンソール画面の **Storage > Object Storage** で **[APIエンドポイント設定]** ボタンをクリックして確認できます。

!!! danger "注意"
    リージョンごとに使用するオブジェクトストレージ API の URL アドレスが異なるため、**API エンドポイント設定**の URL を必ずご確認ください。

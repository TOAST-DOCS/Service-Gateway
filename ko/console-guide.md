...
3. **NHN Cloud 계정**과 앞서 설정한 **API 비밀번호 설정**의 비밀번호를 이용하여 **IaaS API Identify** 서비스용 서비스 게이트웨이에 토큰 발급을 요청합니다.
    * 요청<br>
      아래와 같이 NHN Cloud 계정과 API 비밀번호 설정의 비밀번호를 이용하여 인증 토큰을 요청합니다.
